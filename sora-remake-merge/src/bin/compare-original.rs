//! Per-function body syscall comparison between EVO and original.
//!
//! Uses the same AST walker that the merge tool uses. For each .ing pair,
//! counts `system[5,*]` syscalls per function with an anchor-key breakdown.
//! Reports anywhere EVO totals differ from original (real added/removed
//! lines) and anywhere the anchor distribution shifts (e.g. Letter -> Voiced
//! upgrades — same line count, just re-anchored).

#![expect(
    clippy::print_stdout,
    clippy::print_stderr,
    reason = "this is a CLI analysis tool"
)]

use anyhow::Context;
use anyhow::Result;
use anyhow::anyhow;
use ingert::scena::Body;
use ingert::scena::Expr;
use ingert::scena::Function;
use ingert::scena::Stmt;
use sora_remake_merge::AnchorKey;
use sora_remake_merge::classify_syscall_expr;
use sora_remake_merge::parse_ing;
use std::collections::HashMap;
use std::path::Path;
use std::process::ExitCode;
use walkdir::WalkDir;

const EVO_ROOT: &str = "resources/evo-voice-mod/script_en/scena";
const ORIG_ROOT: &str = "resources/original/script_en/scena";

#[derive(Default, Debug, Clone)]
struct FnCounts {
    by_anchor: HashMap<AnchorKind, usize>,
    total: usize,
}

#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash)]
enum AnchorKind {
    Portrait,
    Voiced,
    Letter,
    Plain,
    MapName,
}

impl AnchorKind {
    fn from(key: &AnchorKey) -> Self {
        match key {
            AnchorKey::Portrait { .. } => Self::Portrait,
            AnchorKey::Voiced(_) => Self::Voiced,
            AnchorKey::Letter => Self::Letter,
            AnchorKey::Plain => Self::Plain,
            AnchorKey::MapName => Self::MapName,
        }
    }
}

fn walk_body(stmts: &[Stmt], out: &mut FnCounts) {
    for stmt in stmts {
        walk_stmt(stmt, out);
    }
}

fn walk_stmt(stmt: &Stmt, out: &mut FnCounts) {
    match stmt {
        Stmt::Expr(e) | Stmt::Set(_, _, e) => walk_expr(e, out),
        Stmt::Return(_, e) | Stmt::PushVar(_, _, e) => {
            if let Some(e) = e {
                walk_expr(e, out);
            }
        }
        Stmt::If(_, cond, then, els) => {
            walk_expr(cond, out);
            walk_body(then, out);
            if let Some(els) = els {
                walk_body(els, out);
            }
        }
        Stmt::While(_, cond, body) => {
            walk_expr(cond, out);
            walk_body(body, out);
        }
        Stmt::Switch(_, scrut, cases) => {
            walk_expr(scrut, out);
            for arm in cases.values() {
                walk_body(arm, out);
            }
        }
        Stmt::Block(stmts) => walk_body(stmts, out),
        Stmt::Debug(_, args) | Stmt::Tailcall(_, _, args) => {
            for arg in args {
                walk_expr(arg, out);
            }
        }
        Stmt::Break | Stmt::Continue => {}
    }
}

fn walk_expr(expr: &Expr, out: &mut FnCounts) {
    match expr {
        Expr::Syscall(_, a, b, args) => {
            for arg in args {
                walk_expr(arg, out);
            }
            if let Some(cls) = classify_syscall_expr(*a, *b, args) {
                *out.by_anchor.entry(AnchorKind::from(&cls.key)).or_default() += 1;
                out.total += 1;
            }
        }
        Expr::Call(_, _, args) => {
            for arg in args {
                walk_expr(arg, out);
            }
        }
        Expr::Unop(_, _, inner) => walk_expr(inner, out),
        Expr::Binop(_, _, l, r) => {
            walk_expr(l, out);
            walk_expr(r, out);
        }
        Expr::Value(_, _) | Expr::Var(_, _) | Expr::Ref(_, _) => {}
    }
}

fn count_fn(f: &Function) -> Option<FnCounts> {
    match &f.body {
        Body::Tree(stmts) => {
            let mut out = FnCounts::default();
            walk_body(stmts, &mut out);
            Some(out)
        }
        Body::Flat(_) | Body::Asm(_) => None,
    }
}

fn fmt_anchor_dist(counts: &FnCounts) -> String {
    let p = counts
        .by_anchor
        .get(&AnchorKind::Portrait)
        .copied()
        .unwrap_or(0);
    let v = counts
        .by_anchor
        .get(&AnchorKind::Voiced)
        .copied()
        .unwrap_or(0);
    let l = counts
        .by_anchor
        .get(&AnchorKind::Letter)
        .copied()
        .unwrap_or(0);
    let pl = counts
        .by_anchor
        .get(&AnchorKind::Plain)
        .copied()
        .unwrap_or(0);
    format!("Portrait={p}, Voiced={v}, Letter={l}, Plain={pl}")
}

fn run() -> Result<()> {
    let evo_root = Path::new(EVO_ROOT);
    let orig_root = Path::new(ORIG_ROOT);
    if !evo_root.exists() {
        anyhow::bail!("EVO root does not exist: {}", evo_root.display());
    }
    if !orig_root.exists() {
        anyhow::bail!("original root does not exist: {}", orig_root.display());
    }

    let mut total_diff: i64 = 0;
    let mut count_diff_fns = 0_usize;
    let mut anchor_diff_fns = 0_usize;
    let mut skipped_asm = 0_usize;
    let mut files_processed = 0_usize;

    for entry in WalkDir::new(evo_root).into_iter().filter_map(Result::ok) {
        if !entry.file_type().is_file() {
            continue;
        }
        if entry.path().extension().is_none_or(|e| e != "ing") {
            continue;
        }
        let rel = entry
            .path()
            .strip_prefix(evo_root)
            .map_err(|e| anyhow!("{e}"))?;
        let orig_path = orig_root.join(rel);
        if !orig_path.exists() {
            continue;
        }
        files_processed += 1;
        let evo_src = std::fs::read_to_string(entry.path())
            .with_context(|| format!("read {}", entry.path().display()))?;
        let orig_src = std::fs::read_to_string(&orig_path)
            .with_context(|| format!("read {}", orig_path.display()))?;
        let evo_scena =
            parse_ing(&evo_src).with_context(|| format!("parse EVO {}", entry.path().display()))?;
        let orig_scena = parse_ing(&orig_src)
            .with_context(|| format!("parse original {}", orig_path.display()))?;

        for (name, evo_fn) in &evo_scena.functions {
            let Some(orig_fn) = orig_scena.functions.get(name) else {
                continue;
            };
            let (Some(evo_c), Some(orig_c)) = (count_fn(evo_fn), count_fn(orig_fn)) else {
                skipped_asm += 1;
                continue;
            };
            let count_differs = evo_c.total != orig_c.total;
            let anchors_differ = evo_c.by_anchor != orig_c.by_anchor;
            if count_differs {
                count_diff_fns += 1;
                total_diff += i64::try_from(evo_c.total).unwrap_or(0)
                    - i64::try_from(orig_c.total).unwrap_or(0);
                println!(
                    "[count diff] {rel}  {name}\n  evo:  total={} ({})\n  orig: total={} ({})",
                    evo_c.total,
                    fmt_anchor_dist(&evo_c),
                    orig_c.total,
                    fmt_anchor_dist(&orig_c),
                    rel = rel.display(),
                );
            } else if anchors_differ {
                anchor_diff_fns += 1;
                println!(
                    "[anchor diff] {rel}  {name}  (total={})\n  evo:  {}\n  orig: {}",
                    evo_c.total,
                    fmt_anchor_dist(&evo_c),
                    fmt_anchor_dist(&orig_c),
                    rel = rel.display(),
                );
            }
        }
    }

    println!("\n--- summary ---");
    println!("Files processed:           {files_processed}");
    println!("Functions w/ count diff:   {count_diff_fns}");
    println!("Functions w/ anchor diff:  {anchor_diff_fns}");
    println!("Skipped (asm/flat body):   {skipped_asm}");
    println!("Net syscall diff EVO-orig: {total_diff:+}");
    Ok(())
}

fn main() -> ExitCode {
    match run() {
        Ok(()) => ExitCode::SUCCESS,
        Err(e) => {
            eprintln!("error: {e:#}");
            ExitCode::FAILURE
        }
    }
}
