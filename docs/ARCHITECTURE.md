...
3. **`Body::Asm` functions.** Ingert's tree-mode decompiler can't always recover a `Body::Tree` from the bytecode. In the current corpus one EVO function (`mp3010_01.ing:QS300_01_00`, a quest-completion cutscene) decompiles to `Body::Asm`, an opaque run of raw bytecode. The body walker only touches `Body::Tree`, so left alone that function would keep its GungHo text. The swap layer detects the (`EVO=Asm`, `Xseed=Tree`) case and clones Xseed's `Body::Tree` in its place so the runtime executes Xseed's text. Cloning alone would silently drop the voice cues EVO added inside the bytecode — this function carries its `11, V` pairs in the body only, not in the calls-table, so the calls-table gate (`evo_calls_have_voice_ids`) can't see them. Instead the swap recovers each dialogue call's voice ID from the asm (reconstructing the arg list from the literal pushes that precede each `CallSystem`) and re-injects it into the clone at the same position, gated on the asm and Xseed dialogue-call counts matching so a cue can't be misplaced. The substitution also adopts Xseed's `Called::Merged` called-table alongside the body, so ingert re-infers a table that matches the substituted code; keeping EVO's asm-derived table produced a `.dat` whose called-table disagreed with its body and hung the game. The result carries Xseed's text with EVO's voice intact and no freeze. If the asm can't be parsed into literal args, the swap falls back to the calls-table gate: substitute only when EVO added no voice there either, otherwise leave the body untouched rather than drop audio. It is logged to `output/_audit/body_substitutions.tsv` alongside the count of re-injected voice IDs.
...
- `Body::Flat(_)` and `Body::Asm(_)` paired with `Xseed=Tree`: the body walker only touches `Body::Tree`, so a non-`Tree` EVO body would otherwise leave that function's runtime text untouched. The swap layer detects the (`EVO=Asm|Flat`, `Xseed=Tree`) case and substitutes Xseed's body wholesale — but a naïve clone drops any voice cues EVO added inside the bytecode, since Xseed's body has none. For an `Asm` body it therefore recovers each dialogue call's voice ID and re-injects it into the clone:
  - **Recover.** Walk the `Vec<Op>` and, for each `CallSystem(5, {0,6,8})`, reconstruct the call's arg list by collecting the literal `Push` ops immediately before it (args are pushed in reverse, so the push nearest the call is `arg[0]`). Classify the reconstructed args to bound the prefix, then locate the `11, V` pair within it — the same `is_int_11(i) && next_is_int(i+1)` test `evo_calls_have_voice_ids` uses, so an in-text or trailing literal `11` can't be mistaken for a voice marker. A non-literal operand in the arg window (a computed value) aborts recovery for the whole function.
  - **Re-inject.** Clone Xseed's `Body::Tree`, then walk its `system[5,{0,6,8}]` calls in source order and splice `11, V` into each voiced call at the recorded index. Gated on the asm and Xseed dialogue-call counts matching, so positional injection can't misplace a cue; on a mismatch (or an abort above) the EVO body is left untouched rather than lose audio.
  - **Adopt Xseed's called-table.** The substitution takes Xseed's `Called::Merged` in place of EVO's `Called::Raw` table (`adopt_xseed_body`), so body and table come from the same source. This is not optional: `compile` writes a `Called::Raw` table to the `.dat` verbatim, with no check that it matches the code (the `apply`/reconcile pass runs only at *decompile* time). Pairing EVO's asm-derived table with Xseed's substituted body produces a `.dat` whose called-table disagrees with its body — e.g. `camera_lookat` with a different arg count — and the engine hangs on it (the "Temp Librarian" scene froze on an infinite load until this was fixed). Adopting `Called::Merged` makes ingert re-infer the table from the substituted body at compile time, so the two always agree; the re-injected voice IDs live only in the body, mirroring how EVO ships voiced calls.
  This is why the gate can't simply read `evo_calls_have_voice_ids(&evo.called)`: `QS300_01_00` carries its `11, V` pairs in the body only, not the calls-table, so the calls-table gate reports no voice and a plain clone would drop every cue. (That helper is still used as the conservative fallback when the asm can't be parsed.) When counting `11, V` in a call, `prefix_len > N` alone isn't enough, because some `[5,0]` calls carry extra integer parameters between `char_id` and the portrait tag (e.g. `system[5,0](11510, 25, "<#E…>", …)`) that aren't voice IDs — the literal `11` marker is what's checked. The substitution and its re-injected voice-ID count are logged to `output/_audit/body_substitutions.tsv`. `mp3010_01.ing:QS300_01_00` exercises this path. `Flat` bodies (none in the current corpus) take the plain gated path without re-injection.
...
- **`Body::Asm` substitution with voice re-injection**: `mp3010_01.ing:QS300_01_00`. The test asserts the body-substitution counter increments to one, that voice IDs were re-injected (the count is non-zero and equals the run total), that the printed output no longer contains an `asm { … }` block, and that the substituted body carries both Xseed's completion text and an EVO voice cue (`11, 78898`). The recompile test below confirms the substituted `Tree` body — with Xseed's `Called::Merged` adopted so the emitted called-table matches the body — recompiles back to `.dat`.
