...
3. **[Xseed Restoration Mod](https://www.nexusmods.com/trailsintheskyfirstchapter/mods/52)** — download the **full Xseed restoration mod (loose files)** variant (**v1.7** — see [Compatibility](#compatibility)) and extract its `script_en\` and `table_en\` folders directly into `<GAME_ROOT>\`. This is where every localisation table (`table_en\*.tbl`) comes from; this mod does not ship them.
